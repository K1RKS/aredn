...
1. Build (or download) `aredn-traceroute-0.1.6-r0.apk`
2. On the node: **Status → Packages** → upload / install with allow-untrusted  
   or: `apk add --allow-untrusted aredn-traceroute-0.1.6-r0.apk`
...
chmod +x build.sh
./build.sh
# → dist/aredn-traceroute-0.1.6-r0.apk
```
