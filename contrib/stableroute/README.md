...
1. Build (or download) `stableroute-0.1.25-r0.apk`
2. On the node: **Status → Packages** → upload / install with allow-untrusted  
   or: `apk add --allow-untrusted stableroute-0.1.25-r0.apk`
...
chmod +x build.sh
./build.sh
# → dist/stableroute-0.1.25-r0.apk
```
...
```text
stableroute(0.1.25-r0): destination nodec  runs 10
