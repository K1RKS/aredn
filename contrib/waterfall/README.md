...
- Start disabled while a session is already running (shows iface + est. time left)
- **ath10k** isolated FFT worker (lock + timeout + local Wi-Fi recovery) + survey fallback
- Modal Done closes cleanly (no grey backdrop)

## Install

1. Build `waterfall-0.2.18-r0.apk`
2. `apk add --allow-untrusted waterfall-0.2.18-r0.apk`
